...
var softcap=function softcap(pts, start, intensity){
    if(pts.lte(start)) return new Decimal(pts);
    return Decimal.log10(start).mul(Decimal.pow(Decimal.log10(pts) / Decimal.log10(start),intensity));
}
...
var checkNaN = function checkNaN(data, checked) {
    for(const prop in checked) {
        if(!data[prop]) {
            console.warn(`[checkNaN] Could not find property ${prop} in data`);
